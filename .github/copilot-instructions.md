...
- ドキュメントはGitHubフレーバーマークダウン形式で記述する
- 図はMermaid形式で作図する
- 継続的にメンテナンスするストックドキュメントは `/docs` 直下に配置する
- 特定のトピックの開発に関するフロードキュメントは `/docs/flow` 配下に配置する
  - タスク消化時に過去のフロードキュメントを参照しないこと
  - これらはユーザーが不要と判断した際に削除される
