...
現在のシステムでは、ログは原則として公開（is_public = 1）であるため、以下の理由により`idx_logs_is_public`インデックスを削除しました：

1. **選択性の低さ**: ほとんどのログが公開であるため、is_publicでのフィルタリングによる絞り込み効果が低い
2. **インデックスメンテナンスコストの削減**: 不要なインデックスを削除することで、INSERT/UPDATE時のオーバーヘッドを削減
3. **シンプルさの維持**: より単純なインデックス構造で管理が容易

また、複合インデックスにもis_publicを含めていません。同様の理由により、インデックスに含めても効果が限定的なためです。
